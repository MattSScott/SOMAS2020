...
func (c *client) calculateDisasterContributionCP(currentTurn uint, currentResource shared.Resources, geography disasters.ArchipelagoGeography, lastDisaster uint) shared.Resources {
	...
	minDistance := math.Sqrt(math.Pow(geography.Islands[shared.Teams["Team1"]].X-predictionInfo.epiX, 2) + math.Pow(geography.Islands[shared.Teams["Team1"]].Y-predictionInfo.epiY, 2))
	if minDistance < 1 {
		minDistance = 1
	}
